...
# I'll probably changes table name to program name, but i'll still need the scope i'd
def sendToChangesTXT(a, scopeId, conn):
    with conn.cursor as cur:
        cfile = open(changesTXTFolder+'/changes.txt', 'a')
        for b in a:
            # Check if domain exists 
            cur = conn.cursor()
            statem = "SELECT * FROM Domains WHERE domainName = \'"+b+"\'"
            cur.execute(statem)
            if cur.fetchone():
                next
            else:
                if b not in changesDomains:
                    ...
                    #Log the domain so it's splitable buy ;
                    #Output
                    #database, domain, scopeId
                    cfile.write('Domain^'+b+' ; '+scopeId+'\n')
...
def selectOutScope(conn, domainRangeId):
    #returns the out of scope strings for each domainRangeId
    cur = conn.cursor()
    statem = "SELECT `domain` FROM programs WHERE Name=\'%s\'"%(cProgram)
    cur.execute(statem)
    b = []
...
def callBrutesubs(a):
    try:
        #All duplicates will be ignored through mysql
        #Clean temp folder and then call the loop to populate 
        ...
        #?Watch out for this within a vpn on the Algolia domains 
...
def main(): 
    ...
    ##!!!Need to go over what the functionality of this is
    if args.c:
        #Find other domains through a certs of other domains 
        FNULL = open(os.devnull, 'w')
        cur = conn.cursor()
        ...
        domainList = []
        inScope = []
        for a in selectInScope(conn, args.c):
            if a[:2] == '*.':
                inScope.append(a[2:])
        for a in domainsSQL:
            # a = re.findall(r"['](.*?)[']", str(x))
    ...
    if args.b:
        inScope = mysqlfunc.programNameToInScopeIds(conn, args.b)
        cur = conn.cursor()
        #Check length of inScope
        #Deleted outScope
        if (len(inScope) == 0):
            print '[-] '+args.b+' InScope length was 0'
        for a in inScope:
            cleanTempFolder()
            ...
            pdb.set_trace()
            # !!! Make sure Brutesubs output gets rid of duplicates... and think about other paths
            for d in returnInScopeIds(cur, 'Yahoo'):
                # Domains that are currently in the domainIdRange
                e = parsingSqlData.domainRangeByrangeId(conn, e)
                # iterate over over c over pop duplicates
                for f in e:
                    for index, g in reversed(list(enumerate(c))):
                        if g == f:
                            c.pop(index)
